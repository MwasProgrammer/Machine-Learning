...
def fetch_members(city, limit=50):
    ...
    all_members = [
        ...
        {"name": "Matiba Njuru", "city": "Mombasa",  "steps": 12500,  "protocol": "OMAD", "sleep": 5.5}
    ]
...
def process_members(members, step_goal):
    ...
    top_sleep = max(members, key=lambda m: m["sleep"]) if members else {}
    return {
        "total": len(members),
        ...
        "top_sleep": top_sleep.get("sleep", 0),
        "top_sleep_name": top_sleep.get("name", "N/A"),
...
def print_report(city, summary):
    ...
    print(f"  Top Sleep:     {summary['top_sleep_name']} ({summary['top_sleep']} hrs)")
